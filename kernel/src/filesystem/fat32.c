...
FS_ERR fat32_cut_clusters_chain(fs_t* fs, uint32_t start_cluster) {
    ...
    // free them
    FS_ERR err = fat32_free_clusters_chain(fs, FAT_val);
...
FS_ERR fat32_read_file(fs_node_t* node, uint8_t* buffer) {
    if(node-> attr & NODE_DIRECTORY) return ERR_FS_NOT_FILE;
...
FS_ERR fat32_write_file(fs_t* fs, uint32_t* start_cluster, uint8_t* buffer, int size, int cluster_offset) {
    ...
    if(cluster_offset > 0) {
        ...
        memcpy(ext_buffer + cluster_offset, buffer, cluster_size - cluster_offset);
        ...
        current_cluster = fat32_expand_clusters_chain(fs, current_cluster, 1);
        if(current_cluster == 0) return ERR_FS_FAILED;
    }

    int estimated_clusters = size / cluster_size;
    if(estimated_clusters > 0)
        fat32_expand_clusters_chain(fs, current_cluster, estimated_clusters);
    ...
    while(size > 0) {
        ...
        if((unsigned)size >= cluster_size)
            ata_pio_LBA28_access(false, fs->partition.LBA_start + first_sector, sectors_per_cluster, buffer + write_time * cluster_size + cluster_offset);
        else {
...
fs_node_t fat32_add_entry(fs_node_t* parent, char* name, uint32_t start_cluster, uint8_t attr, size_t size) {
    ...
    if(start_index != 0) {
        ...
        memset(directory + start_index + 1, 0, cluster_size - start_index);
        uint32_t first_sector = ((current_cluster - 2) * sectors_per_cluster) + first_data_sector;
        ata_pio_LBA28_access(false, parent->fs->partition.LBA_start + first_sector, sectors_per_cluster, directory);
    }
...
FS_ERR fat32_remove_entry(fs_node_t* parent, char* name) {
    ...
    while(true) {
        ...
        for(final_index = 0; (unsigned)final_index < cluster_size; final_index += 32) {
            if(directory[final_index] == 0x00) return ERR_FS_NOT_FOUND;
        ...
        if(FAT_val >= 0x0ffffff8)
            return ERR_FS_NOT_FOUND; // end of cluster
        if(FAT_val == 0x0ffffff7)
            return ERR_FS_BAD_CLUSTER; // bad cluster
    ...
    if(temp_dir.attr & NODE_DIRECTORY) {
        ...
        // read back the original table
        first_sector = ((current_cluster - 2) * sectors_per_cluster) + first_data_sector;
        ata_pio_LBA28_access(true, parent->fs->partition.LBA_start + first_sector, sectors_per_cluster, directory);
    }
    ...
    int clear_val;
    if((unsigned)final_index + 32 < cluster_size) {
        if(directory[final_index+32] == 0x0)
            clear_val = 0x0;
        else clear_val = 0xe5;
    }
    else {
    ...
    if(clear_val == 0xe5 || (clear_val == 0x0 && final_index >= 0)) {
        // delete LFN entries in the current cluster
        memset(directory+(final_index < 0 ? 0 : final_index), clear_val, 32 * (lfn_entry_count + 1));
        // write
    ...
    if(final_index < 0) {
        ...
        // note that last_entry == 0 is very unlikely
        final_index += cluster_size;

        int first_sector = ((last_cluster - 2) * sectors_per_cluster) + first_data_sector;
        ata_pio_LBA28_access(true, parent->fs->partition.LBA_start + first_sector, sectors_per_cluster, directory);

        memset(directory+final_index, clear_val, cluster_size - final_index);

        // write
        // note that first_sector is set (see above)
        ata_pio_LBA28_access(false, parent->fs->partition.LBA_start + first_sector, sectors_per_cluster, directory);

        if(clear_val == 0x0) {
            fat32_free_clusters_chain(parent->fs, current_cluster);
